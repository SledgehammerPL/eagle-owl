...
g_l["YEAR"] = "Select a full year";
g_l["MONTH"] = "Select a full month";
...
JsDatePick.prototype.addOnSelectedMonthDelegate = function(aDelegatedFunction){
	if (typeof(aDelegatedFunction) == "function"){
		this.addonSelectedMonthDelegate = aDelegatedFunction;
	}
	return false;
};

JsDatePick.prototype.setOnSelectedMonthDelegate = function(aDelegatedFunction){
	if (typeof(aDelegatedFunction) == "function"){
		this.onSelectedMonthDelegate = aDelegatedFunction;
		return true;
	}
	return false;
};

JsDatePick.prototype.executeOnSelectedMonthDelegateIfExists = function(){
	if (typeof(this.onSelectedMonthDelegate) == "function"){
		this.onSelectedMonthDelegate();
	}
	if (typeof(this.addonSelectedMonthDelegate) == "function"){
		this.addonSelectedMonthDelegate();
	}
};

JsDatePick.prototype.addOnSelectedYearDelegate = function(aDelegatedFunction){
	if (typeof(aDelegatedFunction) == "function"){
		this.addonSelectedYearDelegate = aDelegatedFunction;
	}
	return false;
};

JsDatePick.prototype.setOnSelectedYearDelegate = function(aDelegatedFunction){
	if (typeof(aDelegatedFunction) == "function"){
		this.onSelectedYearDelegate = aDelegatedFunction;
		return true;
	}
	return false;
};

JsDatePick.prototype.executeOnSelectedYearDelegateIfExists = function(){
	if (typeof(this.onSelectedYearDelegate) == "function"){
		this.onSelectedYearDelegate();
	}
	if (typeof(this.addonSelectedYearDelegate) == "function"){
		this.addonSelectedYearDelegate();
	}
};
...
JsDatePick.prototype.setMonthSelection = function(){
	this.flag_aDayWasSelected = true;
	this.selectedDayObject.month  = this.currentMonth;
	this.selectedDayObject.year  = this.currentYear;
}

JsDatePick.prototype.setYearSelection = function(){
	this.flag_aDayWasSelected = true;
	this.selectedDayObject.year  = this.currentYear;
}
...
JsDatePick.prototype.setControlBarMonth = function(aText){
	var aTextNode = document.createTextNode(aText);
	
	while (this.controlsBarMonthCell.firstChild){
		this.controlsBarMonthCell.removeChild(this.controlsBarMonthCell.firstChild);
	}
	
	this.controlsBarMonthCell.appendChild(aTextNode);
};

JsDatePick.prototype.setControlBarYear = function(aText){
	var aTextNode = document.createTextNode(aText);
	
	while (this.controlsBarYearCell.firstChild){
		this.controlsBarYearCell.removeChild(this.controlsBarYearCell.firstChild);
	}
	
	this.controlsBarYearCell.appendChild(aTextNode);
};
...
JsDatePick.prototype.getDOMControlBar = function(){
	var d = document, controlsBar,monthForwardButton,monthBackwardButton,yearForwardButton,yearBackwardButton,/*controlsBarText,*/monthButton,yearButton;
	...
	monthButton			= d.createElement("div");
	yearButton			= d.createElement("div");
	...
	this.setC(monthButton, "monthButton");
	this.setC(yearButton, "yearButton");
//	this.setC(controlsBarText, "controlsBarText");
	...
	monthButton.setAttribute("globalNumber",this.globalNumber);
	yearButton.setAttribute("globalNumber",this.globalNumber);
	yearForwardButton.setAttribute("globalNumber",this.globalNumber);
	
//	this.controlsBarTextCell = controlsBarText;
	this.controlsBarMonthCell = monthButton;
	this.controlsBarYearCell = yearButton;

	monthTextNode = d.createTextNode(this.monthsTextualRepresentation[this.currentMonth]);
	yearTextNode = d.createTextNode(this.currentYear);
	monthButton.appendChild(monthTextNode);
	yearButton.appendChild(yearTextNode);
	...
	controlsBar.appendChild(monthButton);
	controlsBar.appendChild(yearButton);
//	controlsBar.appendChild(controlsBarText);
	...
	/* year button */
	yearButton.onmouseover = function(){
		var parentElement,gRef;
		if (parseInt(this.getAttribute("isJsDatePickDisabled")) == 1){
			return;
		}
		...
		gRef = JsDatePick.getCalInstanceById(this.getAttribute("globalNumber"));
		gRef.setTooltipText(g_l["YEAR"]);
		gRef.setC(this, "yearButtonOver");
	};
	
	yearButton.onmouseout = function(){
		var parentElement,gRef;
		if (parseInt(this.getAttribute("isJsDatePickDisabled")) == 1){
			return;
		}		
		...
		gRef = JsDatePick.getCalInstanceById(this.getAttribute("globalNumber"));
		gRef.setTooltipText('');
		gRef.setC(this, "yearButton");
	};
	yearButton.onmousedown = function(){
		var parentElement,gRef;
		if (parseInt(this.getAttribute("isJsDatePickDisabled")) == 1){
			return;
		}
	...
	yearButton.onmouseup = function(){
		...
		gRef.setTooltipText(g_l["YEAR"]);
		gRef.setC(this, "yearButton");
		gRef.setYearSelection();
		gRef.executeOnSelectedYearDelegateIfExists();
	};
	...
	monthButton.onmouseover = function(){
		...
		gRef = JsDatePick.getCalInstanceById(this.getAttribute("globalNumber"));
		gRef.setTooltipText(g_l["MONTH"]);
		gRef.setC(this, "monthButtonOver");
	};
	...
	monthButton.onmouseout = function(){
		...
		gRef = JsDatePick.getCalInstanceById(this.getAttribute("globalNumber"));
		gRef.setTooltipText('');
		gRef.setC(this, "monthButton");
	};
	monthButton.onmousedown = function(){
		var parentElement,gRef;
		if (parseInt(this.getAttribute("isJsDatePickDisabled")) == 1){
			return;
		}
		...
		gRef = JsDatePick.getCalInstanceById(this.getAttribute("globalNumber"));
		gRef.setTooltipText(g_l["MONTH"]);
		gRef.setC(this, "monthButtonDown");
	};
	
	monthButton.onmouseup = function(){
		var parentElement,gRef;
		if (parseInt(this.getAttribute("isJsDatePickDisabled")) == 1){
			return;
		}
		...
		gRef.setTooltipText(g_l["MONTH"]);
		gRef.setC(this, "monthButton");
		gRef.setMonthSelection();
		gRef.executeOnSelectedMonthDelegateIfExists();
	};
